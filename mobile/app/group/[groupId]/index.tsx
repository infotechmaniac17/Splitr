...
import { formatMoney, type GroupResponse, type PairwiseBalance } from "@splitr/core";
...
export default function GroupDetailScreen() {
  ...
  async function onAddMember() {
    ...
    try {
      ...
      await recordGroupMember(groupId, { id: user.id, name: user.name, email: user.email });
      setNewMemberId("");
      await load();
    } catch (err) {
    ...
    <Screen refreshing={false} onRefresh={load}>
      ...
      <View style={styles.actionsRow}>
        <Button
          ...
          onPress={() => router.push({ pathname: "/expense/upload", params: { groupId: group.id } })}
        />
      ...
      {balances.length === 0 ? (
        ...
        balances.map((b, idx) => (
          <View key={idx} style={styles.row}>
            ...
            <Text style={styles.rowAmount}>{formatMoney(b.net_amount_minor, "INR")}</Text>
          </View>
        ))
      )}
      ...
      {members.length === 0 ? (
        <Text style={styles.muted}>
          No members cached on this device yet (member list isn't fetchable
          from the backend — only newly-added members show here).
        </Text>
      ) : (
        members.map((m) => (
      ...
      <Field
        ...
        error={memberError}
      />
      <Button title="Add member" variant="secondary" onPress={onAddMember} loading={addingMember} />
    </Screen>
  );
}
