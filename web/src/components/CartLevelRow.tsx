 ...
 */
export function CartLevelRow({ line, currency }: { line: LineItemResponse; currency: string }) {
    ...
    <div className="flex items-center justify-between rounded-lg bg-gray-50 px-3 py-2 text-sm">
      <div>
        ...
        <p className="text-xs text-gray-400">
          {lineItemKindLabels[line.kind]} · {ALLOCATION_LABELS[allocation] ?? allocation}
        </p>
      </div>
      <Money
        minor={line.total_minor}
        currency={currency}
        className={line.total_minor < 0 ? "font-semibold text-red-600" : "font-semibold"}
      />
    </div>
  );
}
